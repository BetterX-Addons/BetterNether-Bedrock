...
export const seedPlantComponent: BlockCustomComponent = {
    onRandomTick({ block }, { params }) {
        const p = params as { max_size: number };
        const max = p.max_size;
        const permutation = block.permutation;
        const fixedLocation = getFixedLocation(block);
        const growth = permutation.getState("betternether:growth") as number;
        if (growth < max) {
...
function getMaxGrowth(block: Block): number {
...
function setSize(block: Block, growth: number, fixedLocation: Vector3) {
